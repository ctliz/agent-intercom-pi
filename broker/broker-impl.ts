...
import { authorize, POLICY_SEMANTICS_VERSION, type PolicyState } from "@ctliz/agent-intercom-core/policy";
import { POLICY_SEMANTICS_HASH } from "@ctliz/agent-intercom-core/vectors";
import type { BossAuthorizationContext } from "@ctliz/agent-intercom-core/boss";
import { writeMessage, createMessageReader } from "./framing.ts";
import { parseIntercomScopeIdForRegistration, sameIntercomScope } from "../protocol-v4/contract.ts";
import {
...
type LifecycleEmissionEvent = {
  contractVersion: 1;
  correlation: string;
  sequence: number;
  eventType: "session_left" | "session_joined";
};

type LifecycleEmissionObserver = (event: LifecycleEmissionEvent) => void;

// Module-private observer slot. The observer is bound ONLY through the internal
// `createBrokerWithObserver` factory below; it is never exposed via the package
// root, export map, public constructor option, environment, config, or CLI.
const observerSlot = new WeakMap<object, LifecycleEmissionObserver>();

export class IntercomBroker {
  private lifecycleEmissionSequence = 0;
  ...
  private handleMessage(
    ...
    switch (clientMessage.type) {
      case "register": {
        if (!isExactClientRegistrationEnvelope(clientMessage)) {
        ...
        try {
          scopeId = parseIntercomScopeIdForRegistration(clientMessage.scopeId);
        } catch {
          this.sendError(socket, "INVALID_REQUEST", "Registration metadata is invalid");
          socket.end();
          break;
        }
        ...
        let replacementObserver: LifecycleEmissionObserver | undefined;
        let replacementCorrelation: string | undefined;
        if (previousLocal) {
          replacementObserver = observerSlot.get(this);
          replacementCorrelation = replacementObserver ? randomUUID() : undefined;
          if (replacementObserver) this.emitLifecycleEmission(replacementObserver, "session_left", replacementCorrelation!);
        ...
        if (replacementObserver) this.emitLifecycleEmission(replacementObserver, "session_joined", replacementCorrelation!);
      ...
      case "list": {
        ...
        const sameScopeSessions = actor
          ? allSessions.filter((candidate) => sameIntercomScope(actor.scopeId, candidate.scopeId))
          : [];
        const sessions = visibleSessions(sameScopeSessions, currentId!);
      ...
      case "send": {
        ...
        const candidates = bossContext
          ...
          : this.findSessions(currentId, clientMessage.to);
  ...
  private handleAccessControl(socket: net.Socket, message: Record<string, unknown>): void {
    ...
    const issued = this.accessRegistry.issueEnrollment({
      ...
      ...(enrollment.canDelegate !== undefined ? { canDelegate: enrollment.canDelegate as boolean } : {}),
      ...(enrollment.maxDepth !== undefined ? { maxDepth: enrollment.maxDepth as number } : {}),
      ...(enrollment.maxChildren !== undefined ? { maxChildren: enrollment.maxChildren as number } : {}),
  ...
  private handleRemoteAccessControl(socket: net.Socket, message: Record<string, unknown>): void {
    if (
      typeof message.requestId !== "string"
      || message.requestId.length > MAX_MESSAGE_ID_LENGTH
    ...
    let parent: RemotePrincipalRecord;
    try {
  ...
  private emitLifecycleEmission(observer: LifecycleEmissionObserver, eventType: "session_left" | "session_joined", correlation: string): void {
    try {
      observer(Object.freeze({ contractVersion: 1, correlation, sequence: ++this.lifecycleEmissionSequence, eventType }));
    } catch {
      // Observer failures must never affect broker behavior.
    }
  }
...
export function createBrokerWithObserver(observer: LifecycleEmissionObserver): IntercomBroker {
  const broker = new IntercomBroker();
  observerSlot.set(broker, observer);
  return broker;
}
